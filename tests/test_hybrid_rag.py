...
def test_dialogue_summarization_comparison():
    ...
    summarizer_old = DialogueSummarizer(
        rag_system=rag_system, use_hybrid=False, streaming=False
    )
    ...
    summarizer_new = DialogueSummarizer(
        hybrid_rag_system=hybrid_rag, use_hybrid=True, streaming=False
    )
    ...
    print(
        f"신뢰도 점수 - 기존: {result_old['confidence_score']:.2f}, 하이브리드: {result_new['confidence_score']:.2f}"
    )
