...
class MedicalTokenizer:
    ...
    def __init__(self):
        ...
        # 의학 약어 사전
        self.medical_abbr = {
            'BP', 'HR', 'ECG', 'MRI', 'CT', 'WBC', 'RBC', 'HGB', 'PLT',
            'AST', 'ALT', 'BUN', 'Cr', 'CRP', 'ESR', 'PT', 'aPTT', 'INR',
            'Na', 'K', 'Cl', 'CO2', 'Ca', 'Mg', 'P', 'Alb', 'T-bil'
        }
        ...
        self.medical_patterns = [
            ...
            r'[A-Z]{2,}',  # 대문자 약어
            r'q\.?d\.?', r'b\.?i\.?d\.?', r't\.?i\.?d\.?', r'q\.?i\.?d\.?',  # 투약 빈도
            r'p\.?o\.?', r'i\.?v\.?', r'i\.?m\.?', r's\.?c\.?',  # 투약 경로
        ]
    ...
    def _load_medical_dictionary(self) -> set:
        ...
        # 기본 의학 용어 추가
        default_terms = {
            '두통', '발열', '기침', '호흡곤란', '흉통', '복통', '구토', '설사',
            '아스피린', '타이레놀', '부루펜', '페니실린', '아목시실린',
            '당뇨', '고혈압', '천식', '폐렴', '위염', '간염'
        }
...
class HybridMedicalRAG:
    ...
    def hybrid_search(
        ...
        # 5. 의학 도메인 리랭킹
        reranked_results = self._rerank_medical(
            combined_scores,
            query,
            top_k
        )
    ...
    def _has_symptom_disease_match(
        self,
        symptoms: List[str],
        document: str
    ) -> bool:
