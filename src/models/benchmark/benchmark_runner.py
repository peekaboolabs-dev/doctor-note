    ...
    def get_metrics(self) -> dict[str, float]:
        ...
        if len(self.tokens) > 1:
            ...
            metrics["tokens_per_second"] = (
                len(self.tokens) / total_time if total_time > 0 else 0
            )
...
class BenchmarkRunner:
    ...
    def run_model_benchmark(
        ...
        for dialogue_data in test_dialogues:
            ...
            try:
                ...
                test_result = {
                    ...
                    "tokens_per_second": output_tokens / execution_time
                    if execution_time > 0
                    else 0,
        ...
        statistics = {
            "success_rate": len(successful_tests) / len(test_dialogues)
            if test_dialogues
            else 0,
            "total_execution_time": sum(
                r["execution_time"] for r in results["results"]
            ),
        }
        ...
        if successful_tests:
            ...
            statistics["latency"] = {
                ...
                "p95": stats.quantiles(execution_times, n=20)[18]
                if len(execution_times) > 1
                else execution_times[0],
                "p99": stats.quantiles(execution_times, n=100)[98]
                if len(execution_times) > 1
                else execution_times[0],
            }
            ...
            if "tokens_per_second" in successful_tests[0]:
                ...
                statistics["tokens"] = {
                    ...
                    "avg_output_per_request": total_output_tokens
            ...
            if "resources" in successful_tests[0]:
                cpu_avgs = [
                    r["resources"]["cpu_percent_avg"]
                    for r in successful_tests
                    if "cpu_percent_avg" in r["resources"]
                ]
                mem_avgs = [
                    r["resources"]["memory_mb_avg"]
                    for r in successful_tests
                    if "memory_mb_avg" in r["resources"]
                ]
                mem_maxs = [
                    r["resources"]["memory_mb_max"]
                    for r in successful_tests
                    if "memory_mb_max" in r["resources"]
                ]
                ...
                # Ollama 리소스 통계
                ollama_mem_avgs = [
                    r["resources"]["ollama_memory_mb_avg"]
                    for r in successful_tests
                    if "ollama_memory_mb_avg" in r["resources"]
                ]
                ollama_mem_maxs = [
                    r["resources"]["ollama_memory_mb_max"]
                    for r in successful_tests
                    if "ollama_memory_mb_max" in r["resources"]
                ]
                ollama_cpu_avgs = [
                    r["resources"]["ollama_cpu_percent_avg"]
                    for r in successful_tests
                    if "ollama_cpu_percent_avg" in r["resources"]
                ]
    ...
    def compare_models(
        self, model_names: list[str], test_dialogues: list[dict[str, Any]]
    ):
        """
        여러 모델을 비교 테스트
        ...
        comparison_file = os.path.join(
            self.output_dir, f"comparison_{comparison_results['comparison_id']}.json"
        )
        ...
        for model, model_stats in comparison_results["model_results"].items():
            print(f"\n{model}:")
            print(f"  - 성공률: {model_stats['success_rate'] * 100:.1f}%")
